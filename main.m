...
misc.dataFolder = 'Input/';

misc.probeAmpFileName   = 'probe_amplitude.tif';
misc.probePhaseFileName = 'probe_phase.tif';
misc.diffImageFileName  = 'diffraction_noise10.tif';
misc.positionFileName   = 'position.csv';

%% set optimization parameter

outhead = 'rec';

numStep = 6;        % decimation
...
initialProbeAmplitude = imread([misc.dataFolder misc.probeAmpFileName]);
initialProbePhase     = imread([misc.dataFolder misc.probePhaseFileName]);
allPositions = readmatrix([misc.dataFolder misc.positionFileName]);

p = double(initialProbeAmplitude) .* exp(1i*double(initialProbePhase));

imageInfo = imfinfo([misc.dataFolder misc.diffImageFileName]);
allData = zeros(imageInfo(1).Height,imageInfo(1).Width,length(imageInfo),'single');
for n = 1:length(imageInfo)
    allData(:,:,n) = imread([misc.dataFolder misc.diffImageFileName],n);
end
clear imageInfo n

%% initialization

[diffData,illumPos] = decimateData(allData,allPositions,numStep);
...
function [deciData,deciPosI] = decimateData(data,posI,numStep)
sizeSQRT = sqrt(size(data,3)); % assuming square sampling (not rectangular)
iV = (1:numStep:sizeSQRT)';
iH = iV';

idx = sub2ind([sizeSQRT sizeSQRT],repmat(iV,[1 length(iH)]),repmat(iH,[length(iV) 1]));
deciData = data(:,:,idx);
deciPosI = posI(idx,:);
end
